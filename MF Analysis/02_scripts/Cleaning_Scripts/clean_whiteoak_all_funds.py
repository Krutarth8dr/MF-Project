...
OUTPUT_FOLDER = PROJECT_ROOT / "03_clean_data" / "WHITEOAK"
OUTPUT_FOLDER.mkdir(parents=True, exist_ok=True)
OUTPUT_FILE = OUTPUT_FOLDER / "WhiteOak_All_Funds_Cleaned.xlsx"

AMC_NAME = "WhiteOak MF"
...
def get_canonical_fund_name(filename):
    ...
    if "multiasset" in f_lower:
        ...
        return "WhiteOak Capital Mid Cap Fund"
    elif "flexicap" in f_lower:
        return "WhiteOak Capital Flexi Cap Fund"
    elif "elss" in f_lower or "taxsaver" in f_lower:
    ...
    for r in range(1, 10):
        for c in range(1, 8):
            ...
            if val and "as on" in str(val).lower():
                ...
                m2 = re.search(r"as\s+on\s+(\d{1,2})(?:st|nd|rd|th)?\s+([a-zA-Z]+),?\s+(\d{4})", text, re.I)
                if m2:
                    day_str, mo_str, yr_str = m2.groups()
                    mo_num = MONTH_MAP.get(mo_str.lower().strip())
                    if mo_num:
                        return datetime(int(yr_str), mo_num, int(day_str))

                m3 = re.search(r"as\s+on\s+([a-zA-Z]+)\s+(\d{4})", text, re.I)
                if m3:
                    mo_str, yr_str = m3.groups()
                    mo_num = MONTH_MAP.get(mo_str.lower().strip())
                    if mo_num:
                        y = int(yr_str)
                        last_d = calendar.monthrange(y, mo_num)[1]
                        return datetime(y, mo_num, last_d)
...
def clean_whiteoak_data():
    print("=" * 75)
    print("WHITEOAK CAPITAL MUTUAL FUND - CLEANING PIPELINE")
    ...
    for file_idx, file_path in enumerate(excel_files, 1):
        fund_name = get_canonical_fund_name(file_path.name)
        if not fund_name:
        ...
        for r in range(1, 20):
            ...
            for c_idx, val in enumerate(row_vals, 1):
                ...
                if "name of the instrument" in v_lower:
                    ...
                    col_isin = c_idx
                elif any(k in v_lower for k in ["industry / rating", "industry/rating", "industry", "rating"]):
                    col_industry = c_idx
