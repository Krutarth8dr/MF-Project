import os
import re
import sys
from datetime import datetime
from pathlib import Path
import openpyxl
import pandas as pd

# ==============================================================================
# SETTINGS & PATHS
# ==============================================================================

PROJECT_ROOT = Path(__file__).resolve().parents[2]
RAW_FOLDER = PROJECT_ROOT / "01_raw_files" / "Edelweiss"
OUTPUT_FOLDER = PROJECT_ROOT / "03_clean_data" / "Edelweiss"
OUTPUT_FOLDER.mkdir(parents=True, exist_ok=True)
OUTPUT_FILE = OUTPUT_FOLDER / "Edelweiss_All_Funds_Cleaned.xlsx"

AMC_NAME = "Edelweiss Mutual Fund"
START_DATE = pd.Timestamp(year=2024, month=10, day=1)
END_DATE = pd.Timestamp(year=2026, month=8, day=31)

STANDARD_COLUMNS = [
    "AMC",
    "Fund_Name",
    "Portfolio_Date",
    "Month",
    "Security_Name",
    "ISIN",
    "Industry_Rating",
    "Quantity",
]

# 15 Target Funds Configuration
TARGET_FUNDS_CONFIG = {
    "EEARFD": "Edelweiss Balanced Advantage Fund",
    "EEBCYF": "Edelweiss Business Cycles Fund",
    "EECONF": "Edelweiss Consumption Fund",
    "EEDGEF": "Edelweiss Large Cap Fund",
    "EEECRF": "Edelweiss Flexi Cap Fund",
    "EEELSS": "Edelweiss ELSS Tax Saver Fund",
    "EEEQTF": "Edelweiss Large & Mid Cap Fund",
    "EEESCF": "Edelweiss Small Cap Fund",
    "EEESSF": "Edelweiss Equity Savings Fund",
    "EEFINS": "Edelweiss Financial Services Fund",
    "EEMAAF": "Edelweiss Multi Asset Allocation Fund",
    "EEMCPF": "Edelweiss Multi Cap Fund",
    "EESMCF": "Edelweiss Mid Cap Fund",
    "EETECF": "Edelweiss Technology Fund",
}


def clean_text(val):
    if val is None:
        return ""
    text = str(val).strip()
    return re.sub(r"\s+", " ", text)


def clean_edelweiss_data():
    print("=" * 70)
    print("EDELWEISS MUTUAL FUND - CLEANING PIPELINE")
    print(f"Raw Folder:   {RAW_FOLDER}")
    print(f"Output File:  {OUTPUT_FILE}")
    print(f"Target Date Range: {START_DATE.strftime('%b %Y')} to {END_DATE.strftime('%b %Y')}")
    print("=" * 70)

    excel_files = sorted(list(RAW_FOLDER.glob("*/*/*.xlsx")))
    if not excel_files:
        print(f"ERROR: No monthly Excel files found in {RAW_FOLDER}")
        sys.exit(1)

    print(f"\n[Step 1/3] Found {len(excel_files)} monthly workbooks to process.")

    all_rows = []

    for fpath in excel_files:
        yr = int(fpath.parent.parent.name)
        mo = int(fpath.parent.name)

        # Calculate exact month-end date
        portfolio_date = pd.Period(f"{yr}-{mo:02d}", freq="M").end_time.date()
        month_str = portfolio_date.strftime("%b-%Y")
        portfolio_dt = pd.to_datetime(portfolio_date)

        if not (START_DATE <= portfolio_dt <= END_DATE):
            continue

        print(f"  Processing {month_str} ({fpath.name})...")
        wb = openpyxl.load_workbook(fpath, data_only=True)
        sheet_names = set(wb.sheetnames)

        # Parse Index sheet for fund name descriptions
        index_names = {}
        if "Index" in sheet_names:
            ws_idx = wb["Index"]
            for row in ws_idx.iter_rows(values_only=True):
                if row and row[0]:
                    code = str(row[0]).strip()
                    desc = str(row[1]).strip() if len(row) > 1 and row[1] else ""
                    m = re.search(r'"([^"]+)"\)', desc)
                    if m:
                        desc = m.group(1)
                    if code and desc:
                        index_names[code] = desc

        for code, canonical_name in TARGET_FUNDS_CONFIG.items():
            if code not in sheet_names:
                continue

            fund_name = TARGET_FUNDS_CONFIG.get(code, index_names.get(code, canonical_name))
            ws = wb[code]

            # Detect header row and column indices
            header_row = -1
            col_instrument = 0
            col_isin = 1
            col_industry = 2
            col_qty = 3

            rows_data = list(ws.iter_rows(values_only=True))
            for r_idx, r_vals in enumerate(rows_data[:15]):
                r_str = [str(v).lower() if v is not None else "" for v in r_vals]
                if any("instrument" in v for v in r_str) and any("isin" in v for v in r_str):
                    header_row = r_idx
                    for c_idx, val in enumerate(r_str):
                        if "instrument" in val:
                            col_instrument = c_idx
                        elif "isin" in val:
                            col_isin = c_idx
                        elif "industry" in val or "rating" in val:
                            col_industry = c_idx
                        elif "quantity" in val or "qty" in val:
                            col_qty = c_idx
                    break

            if header_row == -1:
                header_row = 3

            fund_rows_count = 0
            for row in rows_data[header_row + 1:]:
                if not row or not any(row):
                    continue

                inst_val = clean_text(row[col_instrument]) if col_instrument < len(row) else ""

                # Stop scanning rows in a sheet after "Sub Total" is detected
                if "sub total" in inst_val.lower() or "subtotal" in inst_val.lower():
                    break

                isin_val = clean_text(row[col_isin]) if col_isin < len(row) else ""
                if not isin_val.startswith("INE"):
                    continue

                industry_val = clean_text(row[col_industry]) if col_industry < len(row) else ""

                qty_raw = row[col_qty] if col_qty < len(row) else None
                try:
                    qty = float(qty_raw) if qty_raw is not None else 0.0
                except (ValueError, TypeError):
                    continue

                if qty <= 0:
                    continue

                all_rows.append({
                    "AMC": AMC_NAME,
                    "Fund_Name": fund_name,
                    "Portfolio_Date": portfolio_date.strftime("%Y-%m-%d"),
                    "Month": month_str,
                    "Security_Name": inst_val,
                    "ISIN": isin_val,
                    "Industry_Rating": industry_val,
                    "Quantity": int(round(qty)),
                })
                fund_rows_count += 1

    print("\n[Step 2/3] Compiling cleaned dataset...")
    if not all_rows:
        print("ERROR: No valid equity holdings extracted!")
        sys.exit(1)

    df = pd.DataFrame(all_rows, columns=STANDARD_COLUMNS)

    # Convert types
    df["Portfolio_Date"] = pd.to_datetime(df["Portfolio_Date"])
    df["Quantity"] = df["Quantity"].astype("int64")

    # Sort dataset cleanly
    df = df.sort_values(["Portfolio_Date", "Fund_Name", "Security_Name"]).reset_index(drop=True)

    print(f"Total rows extracted: {len(df):,}")
    print(f"Unique funds:         {df['Fund_Name'].nunique()}")
    print(f"Unique ISINs:         {df['ISIN'].nunique()}")
    print(f"Unique months:        {df['Month'].nunique()}")

    print("\n[Step 3/3] Saving cleaned data to Excel...")
    df.to_excel(OUTPUT_FILE, index=False)
    print(f"Saved successfully: {OUTPUT_FILE} ({OUTPUT_FILE.stat().st_size:,} bytes)")
    print("=" * 70)


if __name__ == "__main__":
    clean_edelweiss_data()
