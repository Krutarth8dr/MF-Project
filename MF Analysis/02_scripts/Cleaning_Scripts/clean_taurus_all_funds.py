...
RAW_FOLDER = PROJECT_ROOT / "01_raw_files" / "TAURUS"
OUTPUT_FOLDER = PROJECT_ROOT / "03_clean_data" / "TAURUS"
OUTPUT_FOLDER.mkdir(parents=True, exist_ok=True)
OUTPUT_FILE = OUTPUT_FOLDER / "Taurus_All_Funds_Cleaned.xlsx"
...
def parse_portfolio_date_from_file(ws, file_path):
    # 1. Search first 10 rows for date patterns
    for r in range(1, 12):
        for c in range(1, ws.max_column + 1):
            val = ws.cell(r, c).value
            if isinstance(val, datetime):
                return val
            if val and "PORTFOLIO STATEMENT AS ON" in str(val).upper():
                text = str(val)
    ...
    # Fallback to directory structure: 01_raw_files/TAURUS/<YYYY>/<MM>/...
...
def parse_taurus_workbook(file_path):
    try:
    ...
    # 1. Scheme Name detection
    fund_name_raw = None
    for r in range(1, 10):
        val = ws.cell(r, 3).value or ws.cell(r, 4).value
        if val and "SCHEME NAME" in str(val).upper():
            for c in range(1, ws.max_column + 1):
                cval = ws.cell(r, c).value
                if cval and "SCHEME NAME" not in str(cval).upper() and "TAURUS" in str(cval).upper():
                    fund_name_raw = clean_text(cval)
                    break
        elif val and "TAURUS" in str(val).upper() and "FUND" in str(val).upper():
            fund_name_raw = clean_text(val)
            break

    fname_combined = (fund_name_raw or "") + " " + file_path.name
    canonical_fund_name = None
    for cname, patterns in TARGET_FUNDS_CONFIG.items():
        if any(p in fname_combined.lower() for p in patterns):
            canonical_fund_name = cname
            break
    ...
    for r in range(1, ws.max_row + 1):
        ...
        row_str = " ".join([clean_text(v) for v in row_vals if v is not None]).lower()
        ...
        if isin_idx is not None:
            ...
            industry = ""
            qty = None
            for i in range(isin_idx + 1, len(row_vals)):
                cell = row_vals[i]
...
def clean_taurus_data():
    print("=" * 75)
    print("TAURUS MUTUAL FUND - CLEANING PIPELINE")
    ...
    for file_idx, file_path in enumerate(excel_files, 1):
        rows = parse_taurus_workbook(file_path)
        if rows:
    ...
    print("\n" + "=" * 75)
    print("TAURUS MUTUAL FUND - CLEANING SUMMARY")
    print("=" * 75)
